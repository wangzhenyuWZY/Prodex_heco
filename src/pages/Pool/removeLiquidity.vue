...
export default {
  ...
  methods: {
    ...
    async getBalance() {
      ...
      contract.methods.balanceOf(this.$store.state.app.defaultAccout).call().then((result)=>{
        that.maxBalance = result
      })
    },
    async approveLpToken() {
      const that = this
      const contract = new web3.eth.Contract(BPool.abi, that.pair.address)
      contract.methods.approve(that.pair.address,'100000000000000000000').send({from:this.$store.state.app.defaultAccout}).then((result)=>{
    ...
    async exitPool() {
      ...
      const contract = new web3.eth.Contract(BPool.abi, that.pair.address)
      contract.methods.exitPool(num.toFixed(),[0, 0]).send({from:this.$store.state.app.defaultAccout}).then((result)=>{
    ...
    async getSpotPrice(address1, address2, name) {
      const that = this
      const contract = new web3.eth.Contract(BPool.abi, that.pair.address)
      contract.methods.getSpotPrice(address1,address2).call().then((result)=>{
        name == 'justPrice' ? this.justPrice = result / Math.pow(10, this.pair.decimals) : this.reversePrice = result / Math.pow(10, this.pair.decimals)
      })
    }
  }
}
