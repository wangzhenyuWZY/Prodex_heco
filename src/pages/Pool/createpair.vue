...
import { Approved, Decimals } from '../../utils/tronwebFn'
import valert from './valret'
import { mapState } from 'vuex'
import {BFactory,WETH,FactoryManager,BPool} from '../../api/deployments'
export default {
  data() {
    return {
      token1: {},
  ...
  methods: {
    ...
    init() { 
      this.getBFactoryContract()
      this.getSwapFeeForDex()
    },
    getBFactoryContract() { // 链接BFactory合约
      this.BFactoryContract = new web3.eth.Contract(BFactory.abi, BFactory.address)
    },
    async getSwapFeeForDex() {
      let that = this
      let FactoryManagerContract = new web3.eth.Contract(FactoryManager.abi, FactoryManager.address)
      FactoryManagerContract.methods.swapFeeForDex().call().then((result)=>{
        var balance = new BigNumber(result);
        balance = balance.div(Math.pow(10, 18)).toFixed(3)
        that.foxDex = balance
      })
    },
    loading1(n) {
    ...
    async createBPool() { // newBPool
      ...
      try {
        that.BFactoryContract.methods.newBPool().send({from:this.$store.state.app.defaultAccout}).then((result)=>{
            that.bPoolContract = result.events.LOG_NEW_POOL.returnValues.pool
            that.setSwapLpFee()
            that.setSponsors()
            Approved(WETH.abi,that.token1.address, that.bPoolContract).then(() => {
              Approved(WETH.abi,that.token2.address, that.bPoolContract).then(() => {
                const number = new BigNumber(that.firstTokenNum * Math.pow(10, that.token1.decimals)).toString(10)
                const weight =new BigNumber(that.firstTokenWeight * Math.pow(10, 18)).toString(10)
                const number2 = new BigNumber(that.secondTokenNum * Math.pow(10, that.token2.decimals)).toString(10)
                const weight2 = new BigNumber(that.secondTokenWeight * Math.pow(10, 18)).toString(10)
    ...
    async setSwapLpFee() { // 设置LP
      ...
      let BPoolContract = new web3.eth.Contract(BPool.abi, that.bPoolContract)
      BPoolContract.methods.setSwapLpFee(this.lp * Math.pow(10, 18)).send({from:this.$store.state.app.defaultAccout}).then((result)=>{
          // that.showAlert1 = true
          that.typeUrl = 'https://shasta.tronscan.org/#/transaction/' + result
      })
    },
    async setSponsors() { // 设置sponsors
      ...
      let BPoolContract = new web3.eth.Contract(BPool.abi, that.bPoolContract)
      BPoolContract.methods.setSwapBuilderFee(this.sponsors * Math.pow(10, 18)).send({from:this.$store.state.app.defaultAccout}).then((result)=>{
          // that.showAlert1 = true
          that.typeUrl = 'https://shasta.tronscan.org/#/transaction/' + result
      })
    },
    async bindCoin(token1Address, token1Balance, token1Weight, token2Address, token2Balance, token2Weight) { // 绑定币种
      const that = this
      let BPoolContract = new web3.eth.Contract(BPool.abi,this.bPoolContract)
      BPoolContract.methods.addToken(token1Address,token1Balance,token1Weight,token2Address,token2Balance,token2Weight).send({from:this.$store.state.app.defaultAccout}).then((result)=>{
          that.showAlert1 = true
          that.loading1(0)
