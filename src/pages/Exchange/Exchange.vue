...
import {WETH,BPool} from '../../api/deployments'
export default {
  data() {
    return {
      token1: {},
  ...
  methods: {
    ...
    async getBalance(token) { // 获取余额
      const that = this
      let BPoolContract = new web3.eth.Contract(WETH.abi, token.address)
      BPoolContract.methods.balanceOf(this.$store.state.app.defaultAccout).call().then((result)=>{
        const balance = (result / Math.pow(10, token.decimals)).toFixed(6)
    ...
    changePair() {
      ...
      Allowance(contract, pair.address).then((res) => {
        if (res) {
          that.approveBalance1 = res/Math.pow(10,that.token1.decimals)
          ...
          Allowance(contract2, pair.address).then((result) => {
            if (res) {
              that.approveBalance2 = result/Math.pow(10,that.token1.decimals)
    ...
    getMinAmountOut() {
      this.tolerance = this.$store.state.app.tolerance
      if (this.tolerance == 0) {
        this.maxPrice = 0
      } else {
    ...
    getBalanceInPool(pair, coin) { // 获取Pool中的余额
      return new Promise(function(resolve, reject) {
        ...
        BPoolContract.methods.getBalance(coin.address).call().then((result)=>{
          ...
          balance = balance.div(Math.pow(10, coin.decimals))
          resolve(balance)
        })
      })
    },
    getWeight(pair, coin) { // 获取权重
      return new Promise(function(resolve, reject) {
        let BPoolContract = new web3.eth.Contract(BPool.abi, pair.address)
        BPoolContract.methods.getDenorm(coin.address).call().then((result)=>{
          const weight = result / Math.pow(10, pair.decimals)
          resolve(weight)
        })
      })
    },
    getSwapFee(pair) { // 获取swapfee
      return new Promise(function(resolve, reject) {
        let BPoolContract = new web3.eth.Contract(BPool.abi, pair.address)
        BPoolContract.methods.getSwapFee().call().then((result)=>{
          const swapFee = result / Math.pow(10, pair.decimals)
          resolve(swapFee)
        })
      })
    },
    ...
    async doswap() {
      ...
      if (parseFloat(that.token1Num) > parseFloat(that.approveBalance1)) {
        that.doApprove()
      }
      ...
      this.btnsbmit = true
      if (parseFloat(that.token2Num) > parseFloat(that.token2Balance)) {
      ...
      let BPoolContract = new web3.eth.Contract(BPool.abi, that.pair.address)
      BPoolContract.methods.swapExactAmountIn(that.token1.address,token1num,that.token2.address,that.maxPrice,MAX).send({from:this.$store.state.app.defaultAccout}).then((result)=>{
